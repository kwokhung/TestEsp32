...
SbrDisplay::SbrDisplay(std::string name)
    : name(name)
{
    task = NULL;
}

void SbrDisplay::setup()
{
    Serial.println("SbrDisplay::setup");

    setup_wifi();
}

void SbrDisplay::setup_wifi()
{
    Serial.println("\nConnecting to Wifi");
    WiFi.begin("xxx", "yyy");
    ...
    server.onNotFound(SbrDisplay::handleNotFound);
    server.on("/", SbrDisplay::displayInfo);
    server.on("/update", SbrDisplay::updateVars);
...
void SbrDisplay::startUp(void *parameter)
{
    SbrDisplay *sbrDisplay = (SbrDisplay *)parameter;
...
void SbrDisplay::handleNotFound()
{
...
ESP32WebServer &SbrDisplay::server = *new ESP32WebServer(80);
